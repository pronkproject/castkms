...
# Metadata must agree with the sanitizer ABI of the kernel crates it imports.
sanitizers=()
if grep -qx 'CONFIG_KASAN_GENERIC=y' "$build/.config"; then
	sanitizers+=(-Zsanitizer=kernel-address)
fi
