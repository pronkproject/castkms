...
static void castkms_frame_dispatch_test_captures_are_counted(struct kunit *test)
{
	struct castkms_frame_dispatch_demand demand = {};
	bool put_vblank;
	bool keep_vblank;
	...
	KUNIT_ASSERT_EQ(test, ret, 0);
	KUNIT_EXPECT_FALSE(test, keep_vblank);
	KUNIT_EXPECT_EQ(test, demand.capture_count, 2);
	ret = castkms_frame_dispatch_demand_get(&demand,
					  CASTKMS_FRAME_DISPATCH_CLIENT_CAPTURE,
					  -EINVAL, &keep_vblank);
	KUNIT_EXPECT_EQ(test, ret, -EINVAL);
	...
	put_vblank = castkms_frame_dispatch_demand_put(&demand,
						 CASTKMS_FRAME_DISPATCH_CLIENT_CAPTURE);
	KUNIT_EXPECT_FALSE(test, put_vblank);
	KUNIT_EXPECT_EQ(test, demand.capture_count, 1);
	put_vblank = castkms_frame_dispatch_demand_put(&demand,
						 CASTKMS_FRAME_DISPATCH_CLIENT_CRC);
	KUNIT_EXPECT_FALSE(test, put_vblank);
	put_vblank = castkms_frame_dispatch_demand_put(&demand,
...
static void castkms_frame_dispatch_test_failed_first_capture(struct kunit *test)
{
	struct castkms_frame_dispatch_demand demand = {};
	bool keep_vblank;
	int ret;
	...
	KUNIT_EXPECT_EQ(test, ret, -EINVAL);
	KUNIT_EXPECT_FALSE(test, keep_vblank);
	KUNIT_EXPECT_EQ(test, demand.capture_count, 0);
	KUNIT_EXPECT_FALSE(test, castkms_frame_dispatch_demand_is_active(&demand));
}
...
static struct kunit_case castkms_frame_dispatch_test_cases[] = {
	...
	KUNIT_CASE(castkms_frame_dispatch_test_captures_are_counted),
	KUNIT_CASE(castkms_frame_dispatch_test_failed_first_capture),
