...
/**
 * struct castkms_configfs_device - Configfs representation of a CASTKMS device
 *
 * @group: Top level configuration group that represents a CASTKMS device.
 * Initialized when a new directory is created under "/config/castkms/"
 * @planes_group: Default subgroup of @group at "/config/castkms/planes"
 ...
 * option changes
 */
struct castkms_configfs_device {
...
/**
 * struct castkms_configfs_plane - Configfs representation of a plane
 *
 * @group: Top level configuration group that represents a plane.
 * Initialized when a new directory is created under "/config/castkms/planes"
 * @possible_crtcs_group: Default subgroup of @group at "plane/possible_crtcs"
 * @dev: The castkms_configfs_device this plane belongs to
 * @config: Configuration of the CASTKMS plane
 */
struct castkms_configfs_plane {
	struct config_group group;
	struct config_group possible_crtcs_group;
	struct castkms_configfs_device *dev;
	struct castkms_config_plane *config;
};

/**
 * struct castkms_configfs_crtc - Configfs representation of a CRTC
 *
 * @group: Top level configuration group that represents a CRTC.
 * Initialized when a new directory is created under "/config/castkms/crtcs"
 * @dev: The castkms_configfs_device this CRTC belongs to
 * @config: Configuration of the CASTKMS CRTC
 */
struct castkms_configfs_crtc {
	struct config_group group;
	struct castkms_configfs_device *dev;
	struct castkms_config_crtc *config;
};

/**
 * struct castkms_configfs_encoder - Configfs representation of a encoder
 *
 * @group: Top level configuration group that represents a encoder.
 * Initialized when a new directory is created under "/config/castkms/encoders"
 * @possible_crtcs_group: Default subgroup of @group at "encoder/possible_crtcs"
 * @dev: The castkms_configfs_device this encoder belongs to
 * @config: Configuration of the CASTKMS encoder
 */
struct castkms_configfs_encoder {
	struct config_group group;
	struct config_group possible_crtcs_group;
	struct castkms_configfs_device *dev;
	struct castkms_config_encoder *config;
};

/**
 * struct castkms_configfs_connector - Configfs representation of a connector
 *
 * @group: Top level configuration group that represents a connector.
 * Initialized when a new directory is created under "/config/castkms/connectors"
 * @possible_encoders_group: Default subgroup of @group at
 * "connector/possible_encoders"
 * @dev: The castkms_configfs_device this connector belongs to
 * @config: Configuration of the CASTKMS connector
 */
struct castkms_configfs_connector {
	struct config_group group;
...
#define child_group_to_castkms_configfs_device(group) \
	device_item_to_castkms_configfs_device((&(group)->cg_item)->ci_parent)

#define plane_item_to_castkms_configfs_plane(item) \
	container_of(to_config_group((item)), struct castkms_configfs_plane, group)

#define plane_possible_crtcs_item_to_castkms_configfs_plane(item) \
	container_of(to_config_group((item)), struct castkms_configfs_plane, \
		     possible_crtcs_group)

#define crtc_item_to_castkms_configfs_crtc(item) \
	container_of(to_config_group((item)), struct castkms_configfs_crtc, group)

#define encoder_item_to_castkms_configfs_encoder(item) \
	container_of(to_config_group((item)), struct castkms_configfs_encoder, \
		     group)

#define encoder_possible_crtcs_item_to_castkms_configfs_encoder(item) \
	container_of(to_config_group((item)), struct castkms_configfs_encoder, \
		     possible_crtcs_group)

#define connector_item_to_castkms_configfs_connector(item) \
	container_of(to_config_group((item)), struct castkms_configfs_connector, \
		     group)

#define connector_possible_encoders_item_to_castkms_configfs_connector(item) \
	container_of(to_config_group((item)), struct castkms_configfs_connector, \
		     possible_encoders_group)

static ssize_t crtc_writeback_show(struct config_item *item, char *page)
{
	struct castkms_configfs_crtc *crtc;
	bool writeback;

	crtc = crtc_item_to_castkms_configfs_crtc(item);

	scoped_guard(mutex, &crtc->dev->lock)
		writeback = castkms_config_crtc_get_writeback(crtc->config);
				    ...
				    size_t count)
{
	...
	scoped_guard(mutex, &crtc->dev->lock) {
		...
		castkms_config_crtc_set_writeback(crtc->config, writeback);
	}
					    ...
					    const char *name)
{
	...
	dev = child_group_to_castkms_configfs_device(group);
	...
	scoped_guard(mutex, &dev->lock) {
		...
		crtc->config = castkms_config_create_crtc(dev->config);
...
{
	struct castkms_configfs_plane *plane;
	struct castkms_configfs_crtc *crtc;
	int ret;
	...
	scoped_guard(mutex, &plane->dev->lock) {
		...
		ret = castkms_config_plane_attach_crtc(plane->config, crtc->config);
	}
...
static ssize_t plane_type_show(struct config_item *item, char *page)
{
	...
	plane = plane_item_to_castkms_configfs_plane(item);

	scoped_guard(mutex, &plane->dev->lock)
		type = castkms_config_plane_get_type(plane->config);
				...
				size_t count)
{
	...
	scoped_guard(mutex, &plane->dev->lock) {
		...
		castkms_config_plane_set_type(plane->config, type);
	}
					     ...
					     const char *name)
{
	...
	scoped_guard(mutex, &dev->lock) {
		...
		plane->config = castkms_config_create_plane(dev->config);
...
static int encoder_possible_crtcs_allow_link(struct config_item *src,
					     struct config_item *target)
{
	struct castkms_configfs_encoder *encoder;
	struct castkms_configfs_crtc *crtc;
	int ret;
	...
	scoped_guard(mutex, &encoder->dev->lock) {
		...
		ret = castkms_config_encoder_attach_crtc(encoder->config, crtc->config);
	}
...
static void encoder_release(struct config_item *item)
{
	...
	encoder = encoder_item_to_castkms_configfs_encoder(item);
					       ...
					       const char *name)
{
	...
	scoped_guard(mutex, &dev->lock) {
		...
		encoder->config = castkms_config_create_encoder(dev->config);
...
{
	struct castkms_configfs_connector *connector;
	enum drm_connector_status status;

	connector = connector_item_to_castkms_configfs_connector(item);

	scoped_guard(mutex, &connector->dev->lock)
		status = castkms_config_connector_get_status(connector->config);
				      ...
				      const char *page, size_t count)
{
	...
	scoped_guard(mutex, &connector->dev->lock) {
		castkms_config_connector_set_status(connector->config, status);

		if (connector->dev->enabled)
			castkms_trigger_connector_hotplug(connector->dev->config->dev);
	}
						  ...
						  struct config_item *target)
{
	...
	connector = connector_possible_encoders_item_to_castkms_configfs_connector(src);
	...
	scoped_guard(mutex, &connector->dev->lock) {
		...
		ret = castkms_config_connector_attach_encoder(connector->config,
							   encoder->config);
	}
						 ...
						 const char *name)
{
	...
	scoped_guard(mutex, &dev->lock) {
		...
		connector->config = castkms_config_create_connector(dev->config);
				    ...
				    size_t count)
{
	...
	scoped_guard(mutex, &dev->lock) {
		if (!dev->enabled && enabled) {
			if (!castkms_config_is_valid(dev->config))
				return -EINVAL;

			ret = castkms_create(dev->config);
			if (ret)
				return ret;
					      ...
					      const char *name)
{
	...
	dev->config = castkms_config_create(name);
...
static struct configfs_subsystem castkms_subsys = {
	.su_group = {
		.cg_item = {
			.ci_name = "castkms",
	...
	.su_mutex = __MUTEX_INITIALIZER(castkms_subsys.su_mutex),
};

int castkms_configfs_register(void)
{
	int ret;
	...
	config_group_init(&castkms_subsys.su_group);
	ret = configfs_register_subsystem(&castkms_subsys);
