...
//! Native probe publication retains completion and reports its result.
...
mod cases {
    ...
    fn unsubmitted_probe_has_no_result() -> Result {
        ...
        assert_eq!(probe.completed(), Err(ENODATA));
        Ok(())
    }
    ...
    fn synchronous_private_probe_completes_without_content_identity() -> Result {
        let probe = probe()?;
        probe.submit_then(None, || Ok(()))?;
        assert_eq!(probe.result(), Ok(true));
        assert_eq!(probe.completed(), Ok(()));
        Ok(())
    }
    ...
    fn native_completion_remains_pending_until_the_fence_signals() -> Result {
        ...
        probe.submit_then(Some(completion.fence()), || Ok(()))?;
        assert_eq!(probe.result(), Ok(false));
        assert_eq!(probe.completed(), Err(EAGAIN));
    ...
    fn native_failure_does_not_become_a_successful_probe() -> Result {
        ...
        assert_eq!(probe.completed(), Err(EIO));
        Ok(())
    }
    ...
    fn validation_failure_returns_the_submission_slot() -> Result {
        let probe = probe()?;
        assert_eq!(
            probe.submit_then(None, || Err(ESTALE)),
            Err(ESTALE)
        );
        probe.submit_then(None, || Ok(()))?;
        assert_eq!(probe.completed(), Ok(()));
    ...
    #[test]
    fn one_endpoint_cannot_publish_two_probe_submissions() -> Result {
        let probe = probe()?;
        probe.submit_then(None, || Ok(()))?;
        assert_eq!(
            probe.submit_then(None, || Ok(())),
