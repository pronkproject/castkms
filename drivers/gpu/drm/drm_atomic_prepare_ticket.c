...
static struct drm_prepare_ticket *ticket_alloc(struct drm_prepare_retirement_set *set)
{
				...
				unsigned int count)
{
	...
	outputs = drm_prepare_outputs_create(entries, count);
	if (IS_ERR(outputs))
		return ERR_CAST(outputs);
	set = drm_prepare_outputs_hold(outputs);
	if (IS_ERR(set)) {
		ticket = ERR_CAST(set);
		goto destroy_outputs;
	}
	ticket = ticket_alloc(set);
...
static void ticket_free(struct kref *ref)
{
	...
	if (ticket->outputs)
		drm_prepare_outputs_destroy(ticket->outputs);
			  ...
			  struct drm_prepare_retirement_guard **guard)
{
	...
	error = drm_prepare_outputs_validate(ticket->outputs, observed, count);
	if (error)
		goto unlock;
	error = install(data);
	if (error)
